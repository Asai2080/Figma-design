...
function buildAssetSvgPrompt({ prompt, name, width, height }) {
  return [
    "You are a senior icon illustrator and SVG engineer specializing in faithful vector recreation of app illustration assets.",
    "Use the attached sliced UI asset as the source of truth.",
    "Create a faithful, polished, editable SVG recreation for Figma. This is a 1:1 similarity-first redraw, not a redesign, restyle, simplification, or icon-set normalization.",
    prompt || `Redraw "${name}" as an SVG asset.`,
    "",
    "Before drawing, silently inspect the reference and build a visual inventory:",
    "- Count the major silhouette lobes/parts and preserve their relative sizes and uneven organic contour.",
    "- Locate all facial features, accessory pieces, decorative marks, motion lines, highlights, shadows, and small supporting shapes.",
    "- Preserve the exact visual relationships: overlap order, angles, spacing, scale, asymmetry, and crop padding.",
    "- Do not output this inventory; use it only to guide the SVG.",
    "",
    "Fidelity goals:",
    "- Match the source asset's silhouette, proportions, pose, orientation, expression, color palette, visual weight, material softness, and emotional tone.",
    "- Preserve distinctive details: irregular puffy outlines, organic curves, inner highlights, soft shaded patches, facial features, accessories, motion streaks, sparkles, small decorations, layered blobs, cast shadows, and asymmetrical contours when present.",
    "- Preserve accessory geometry and attitude. For example, sunglasses, badges, hands, arrows, stars, or facial marks must keep their source angle, thickness, placement, and expression.",
    "- Do not replace the source with a generic icon. Do not flatten a playful illustration into an overly simple symbol. Do not make it cleaner by deleting personality details.",
    ...
    "- Use vector-friendly approximations for soft details: cubic Bezier paths, layered translucent shapes, radial/linear gradients, opacity, and grouped paths.",
    "- Prefer fewer purposeful shapes over pixel-trace fragments, but keep enough layered shapes to preserve the original illustration richness.",
...
function buildAssetSvgRetryPrompt(basePrompt) {
  return [
    ...
    "- Do not trace pixels mechanically, but do preserve the source composition, contour irregularity, accessory angles, expression, decorative details, and soft shading.",
    "- Do not redesign it into a generic simplified icon or remove the small details that make the source recognizable.",
